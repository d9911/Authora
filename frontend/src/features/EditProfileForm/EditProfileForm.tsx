...
export function EditProfileForm() {
  ...
  useEffect(() => {
    setForm((f) => ({
      ...
      avatarUrl: user?.avatarUrl ?? f.avatarUrl,
    }));
  }, [user]);
  ...
  return (
    <form onSubmit={onSubmit} className="card">
      ...
      <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <InputMain label="Avatar URL" value={form.avatarUrl} onChange={set('avatarUrl')} />
        <InputMain label="Cover image URL" value={form.coverSrc} onChange={set('coverSrc')} />
